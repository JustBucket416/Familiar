...
class ExtensionManager private constructor(context: Context) {
    ...
    val extensionHolders = UnsafeHashMap<String, ExtensionHolder>()
        ...
        packages.forEach { packageInfo ->
            ...
            features?.forEach {
                if (it.name == EXTENSION_FEATURE_NAME) {
                    ...
                    try {
                        applicationInfo = pm.getApplicationInfo(packageInfo.packageName, PackageManager.GET_META_DATA)
                    ...
                    val extensionItem = loadExtension(applicationInfo, configuratorName, loaderName, locatorName)
